...
from ._types import MessageT
from ._utils import camel_case, is_optional, pascal_case, safe_snake_case, snake_case
from .runtime.client import ServiceStub as ServiceStub
...
def _get_casing_fn(casing: Casing) -> Callable[[str], str]:
    if casing == "camel":
        return camel_case
    elif casing == "snake":
        return snake_case
...
# CAMEL = stringutils.camel_case  #: A camelCase sterilization function.
# SNAKE = stringutils.snake_case  #: A snake_case sterilization function.
...
def dataclass_field(
    ...
    *,
    ...
    default: Any = None if optional else PLACEHOLDER
...
class ProtoClassMetadata:
    ...
    def __init__(self, cls: Type["Message"]):
        ...
        self.default_gen = _get_default_gen(cls, fields)
...
class Message(ABC):
    ...
    def __post_init__(self) -> None:
        ...
        for field_name, meta in self._cbiproto.meta_by_field_name.items():
    ...
    def __eq__(self, other) -> bool:
        ...
        for field_name in self._cbiproto.meta_by_field_name:
    ...
    def __repr__(self) -> str:
        parts = [
            ...
            for field_name in self._cbiproto.sorted_field_names
    ...
    def __setattr__(self, attr: str, value: Any) -> None:
        ...
        if hasattr(self, "_group_current"):  # __post_init__ had already run
            if attr in self._cbiproto.oneof_group_by_field:
                group = self._cbiproto.oneof_group_by_field[attr]
                for field in self._cbiproto.oneof_field_by_group[group]:
                    if field.name == attr:
    ...
    def __deepcopy__(self: MessageT, _: Any = {}) -> MessageT:
        kwargs = {}
        for name in self._cbiproto.sorted_field_names:
    ...
    @property
    def _cbiproto(self) -> ProtoClassMetadata:
        """
        Lazy initialize metadata for each protobuf class.
        It may be initialized multiple times in a multi-threaded environment,
        but that won't affect the correctness.
        """
        meta = getattr(self.__class__, "_cbiproto_meta", None)
        if not meta:
            meta = ProtoClassMetadata(self.__class__)
            self.__class__._cbiproto_meta = meta  # type: ignore
        return meta
    ...
    def _get_field_default(self, field_name: str) -> Any:
        with warnings.catch_warnings():
            ...
            return self._cbiproto.default_gen[field_name]()
    ...
    def _postprocess_single(self, wire_type: int, meta: FieldMetadata, field_name: str, value: Any) -> Any:
        ...
        if wire_type == WIRE_VARINT:
            ...
            if meta.proto_type == TYPE_STRING:
                ...
                cls = self._cbiproto.cls_by_field[field_name]
                ...
                value = self._cbiproto.cls_by_field[field_name]().parse(value)
    ...
    def parse(self: MessageT, data: bytes) -> MessageT:
        """
    ...
    # For compatibility with other libraries.
    @classmethod
    def FromString(cls: Type[MessageT], data: bytes) -> MessageT:
        """
        Parse the binary encoded Protobuf into this message instance. This
        returns the instance itself and is therefore assignable and chainable.
    ...
    def to_dict(self, casing: Casing = "camel", include_defaults: bool = False) -> Dict[str, Any]:  # noqa: C901
        casing_func = _get_casing_fn(casing)

        output: Dict[str, Any] = {}
        field_types = self._type_hints()
        defaults = self._cbiproto.default_gen
        for field_name, meta in self._cbiproto.meta_by_field_name.items():
            ...
            if meta.proto_type == TYPE_MESSAGE:
                if isinstance(value, datetime):
                    ...
                    if cls == datetime:
                        ...
                        value = [i.to_dict(casing, include_defaults) for i in value]
                    ...
                    output[cased_name] = value.to_dict(casing, include_defaults)
            elif meta.proto_type == TYPE_MAP:
                output_map = {**value}
                for k in value:
                    if hasattr(value[k], "to_dict"):
                        output_map[k] = value[k].to_dict(casing, include_defaults)
    ...
    def from_dict(self: MessageT, value: Dict[str, Any]) -> MessageT:  # noqa: C901
        """
        ...
        for key in value:
            ...
            meta = self._cbiproto.meta_by_field_name.get(field_name)
            if not meta:
                continue
            ...
            if value[key] is not None:
                if meta.proto_type == TYPE_MESSAGE:
                    ...
                    v = getattr(self, field_name)
                    cls = self._cbiproto.cls_by_field[f"{field_name}.value"]
                    for k in value[key]:
                    ...
                    if meta.proto_type in INT_64_TYPES:
                        ...
                        enum_cls = self._cbiproto.cls_by_field[field_name]
                        if isinstance(v, list):
    ...
    def from_json(self: MessageT, value: Union[str, bytes]) -> MessageT:
    ...
    def from_pydict(self: MessageT, value: Dict[str, Any]) -> MessageT:
        self._serialized_on_wire = True
        for key in value:
            field_name = safe_snake_case(key)
            meta = self._cbiproto.meta_by_field_name.get(field_name)
    ...
    def is_set(self, name: str) -> bool:
        default = PLACEHOLDER if not self._cbiproto.meta_by_field_name[name].optional else None
...
__all__ = ["ServiceStub", "Message", "Enum"]
# mkinit
