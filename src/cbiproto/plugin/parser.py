...
from ..lib.google.protobuf import DescriptorProto, EnumDescriptorProto, FileDescriptorProto, ServiceDescriptorProto
...
def generate_code(request: CodeGeneratorRequest) -> CodeGeneratorResponse:
    ...
    sys.stderr.write(f"\033[31mPlugin options: {plugin_options}\033[0m\n")
    ...
    for output_package_name, output_package in request_data.output_packages.items():
        for proto_input_file in output_package.input_files:
            for item, path in traverse(proto_input_file):
                read_protobuf_type(
                    source_file=proto_input_file, item=item, path=path, output_package=output_package,
                )
...
def read_protobuf_type(
    ...
    if isinstance(item, DescriptorProto):
        if item.options.map_entry:
            ...
            return
        # Process Message
        message_data = MessageCompiler(source_file=source_file, parent=output_package, proto_obj=item, path=path)
        ...
        EnumDefinitionCompiler(source_file=source_file, parent=output_package, proto_obj=item, path=path)


def read_protobuf_service(service: ServiceDescriptorProto, index: int, output_package: OutputTemplate) -> None:
    service_data = ServiceCompiler(parent=output_package, proto_obj=service, path=[6, index])
