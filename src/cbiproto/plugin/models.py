...
from cbiproto.plugin.importing import get_type_reference, parse_source_type_name
...
def get_comment(proto_file: "FileDescriptorProto", path: List[int], indent: int = 4) -> str:
    pad = " " * indent
    for sci_loc in proto_file.source_code_info.location:
        if list(sci_loc.path) == path and sci_loc.leading_comments:
            lines = textwrap.wrap(sci_loc.leading_comments.strip().replace("\n", ""), width=79 - indent)
...
class ProtoContentBase:
    ...
    def comment(self) -> str:
        ...
        for this object.
        """
        return get_comment(proto_file=self.source_file, path=self.path, indent=self.comment_indent)
...
class FieldCompiler(MessageCompiler):
    ...
    def get_field_string(self, indent: int = 4) -> str:
        ...
        return f"{name}{annotations} = {cbiproto_field_type}"
    ...
    def field_wraps(self) -> Optional[str]:
        """Returns cbiproto wrapped field type or None."""
...
class ServiceMethodCompiler(ProtoContentBase):
    ...
    def __post_init__(self) -> None:
        ...
        self.output_file.typing_imports.add("Optional")
        self.output_file.imports_type_checking_only.add("from cbiproto.grpc.client import MetadataLike")
        self.output_file.imports_type_checking_only.add("from grpclib.metadata import Deadline")
    ...
    def py_input_message(self) -> Optional[MessageCompiler]:
        ...
        for msg in self.request.all_messages:
            if msg.py_name == name.replace(".", "") and msg.output_file.package == package:
                return msg
        return None
