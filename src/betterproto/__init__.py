...
class Message(ABC):
    ...
    _betterproto_meta: ClassVar[Optional[ProtoClassMetadata]] = None
    ...
    def __post_init__(self) -> None:
        ...
        for field_name, meta in self.betterproto_meta.meta_by_field_name.items():
            if meta.group:
    ...
    def __eq__(self, other: Message) -> bool:
        ...
        for field_name in self.betterproto_meta.meta_by_field_name:
            self_val = self.__raw_get(field_name)
            other_val = other.__raw_get(field_name)
            if self_val is UNSET:
                if other_val is UNSET:
                    continue
                self_val = self.betterproto_meta.get_field_default(field_name)
            elif other_val is UNSET:
                other_val = other.betterproto_meta.get_field_default(field_name)
    ...
    def __repr__(self) -> str:
        parts = [
            ...
            for field_name in self.betterproto_meta.sorted_field_names
    ...
    if not TYPE_CHECKING:
        ...
        def __getattribute__(self, name: str) -> Any:
            ...
            value = self.betterproto_meta.get_field_default(name)
    ...
    def __setattr__(self, attr: str, value: Any) -> None:
        ...
        if hasattr(self, "_group_current"):  # __post_init__ had already run
            if attr in self.betterproto_meta.oneof_group_by_field:
                group = self.betterproto_meta.oneof_group_by_field[attr]
                for field in self.betterproto_meta.oneof_field_by_group[group]:
                    if field.name == attr:
    ...
    def __bool__(self) -> bool:
        return any(
            self.__raw_get(field_name) not in (UNSET, self.betterproto_meta.get_field_default(field_name))
            for field_name in self.betterproto_meta.meta_by_field_name
        )

    def __deepcopy__(self: MessageT, _: Any = {}) -> MessageT:
        kwargs = {}
        for name in self.betterproto_meta.sorted_field_names:
    ...
    def is_set(self, name: str) -> bool:
        default = UNSET if not self.betterproto_meta.meta_by_field_name[name].optional else None
        return self.__raw_get(name) is not default

    @classmethod
    @property
    def betterproto_meta(cls) -> ProtoClassMetadata:
        if not cls._betterproto_meta:
...
def serialize_to_pydict(message: Message, casing: Casing = "camel", include_defaults: bool = False) -> Dict[str, Any]:
    ...
    output: Dict[str, Any] = {}
    defaults = message.betterproto_meta.default_gen
    for field_name, meta in message.betterproto_meta.meta_by_field_name.items():
        ...
        if meta.proto_type == TYPE_MESSAGE:
            ...
            value != message.betterproto_meta.get_field_default(field_name)
            or include_defaults
            or message._include_default_value_for_oneof(field_name=field_name, meta=meta)
        ):
            output[cased_name] = value
...
def deserialize_from_pydict(class_or_instance: Union[MessageT, Type[MessageT]], /, value: Dict[str, Any]) -> MessageT:
    ...
    for key in value:
        ...
        meta = message.betterproto_meta.meta_by_field_name.get(field_name)
        if not meta:
            continue
        ...
        if value[key] is not None:
            if meta.proto_type == TYPE_MESSAGE:
                ...
                if isinstance(v, list):
                    cls = message_cls.betterproto_meta.cls_by_field[field_name]
                ...
                v = getattr(message, field_name)
                cls = message_cls.betterproto_meta.cls_by_field[f"{field_name}.value"]
                cls = cast(Type[MessageT], cls)
    ...
    for field_name, meta in message.betterproto_meta.meta_by_field_name.items():
        ...
        if meta.proto_type == TYPE_MESSAGE:
            if isinstance(value, datetime):
                ...
                # Convert each item.
                cls = message.betterproto_meta.cls_by_field[field_name]
                if cls == datetime:
        ...
        if value[key] is not None:
            if meta.proto_type == TYPE_MESSAGE:
                ...
                if meta.proto_type in INT_64_TYPES:
                    ...
                    enum_cls = message_cls.betterproto_meta.cls_by_field[field_name]
...
def serialize_to_bytes(message: Message) -> bytes:
    ...
    for field_name, meta in message.betterproto_meta.meta_by_field_name.items():
        ...
        if value == message.betterproto_meta.get_field_default(field_name) and not (
...
def deserialize_from_bytes(cls: Union[MessageT, Type[MessageT]], /, data: bytes) -> MessageT:
    ...
    message._serialized_on_wire = True
    proto_meta = message_cls.betterproto_meta
